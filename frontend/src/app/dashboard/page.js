...
export default function Dashboard() {
  ...
  // Search debounce state (FIX: prevents API call on every keystroke)
  ...
  // FIX: Debounce search — only fires API call 400ms after user stops typing
  useEffect(() => {
  ...
  // Fetch patients when search/gender changes
  useEffect(() => {
    if (!user) return;
  ...
  // Fetch doctors dropdown on mount
  useEffect(() => {
    if (!user) return;
    fetchDoctorsDropdown();
  }, []);

  // Fetch doctor worklist when doctors list loads
  useEffect(() => {
    if (!user) return;
            ...
            <div className="grid gap-8 lg:grid-cols-3">
              ...
              <div className="lg:col-span-2 space-y-6">
                <div className="glass p-6 rounded-2xl shadow-md border border-slate-200 dark:border-slate-800">
                  ...
                  {/* FIX: Search uses patientSearchInput for display, debounced patientSearch triggers API */}
